...
struct mm_struct {
	struct {
		...
		// Kernel instance ID of the mm_struct
		u8 mm_ki_id;
#endif
...
#ifdef CONFIG_KERNEL_INSTANCE
// Check if the mm_struct is a ghost instance
static inline bool mm_is_ghost_ki(const struct mm_struct *mm)
{
	return mm && READ_ONCE(mm->mm_ki_id) == 1;
}
#endif
