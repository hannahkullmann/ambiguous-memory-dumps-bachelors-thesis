...
void resched_cpu(int cpu)
{
	...
	// Get the rq for the current kernel instance
	struct rq *rq = ki_rq(current_ki(), cpu);
#else
	       ...
	       struct task_struct *next, struct rq_flags *rf)
{
	...
	#ifdef CONFIG_KERNEL_INSTANCE
	// Update the per-CPU mapping for the kernel instances
	this_cpu_write(this_ki, next->ki ? next->ki : boot_kernel_instance);
	#endif
	...
	this_cpu_write(this_ki, current->ki ? current->ki : boot_kernel_instance);

	// Check if the task's kernel instance matches the current kernel instance
	if (unlikely(task_ki(current) != current_ki())) {
		pr_warn_ratelimited(
			"KI mismatch: task %s[%d] task_ki=%d current_ki=%d\n",
			current->comm, current->pid,
			task_ki(current)->id, current_ki()->id);
	}
	#endif
...
keep_resched:
	...
	if (likely(is_switch)) {
		...
		#ifdef CONFIG_KERNEL_INSTANCE
			// Update the current task of the current kernel instance
			if (likely(current_ki())) {
					current_ki()->current_task = current;
			}
		#endif
