	...
	struct user_namespace *user_ns)
{
	...
	// Set the kernel instance ID of the mm_struct
	mm->mm_ki_id = (u8)task_ki(p)->id;
#endif
...
#ifdef CONFIG_KERNEL_INSTANCE
	// Inherit kernel instance from parent; default to boot instance.
	p->ki = current->ki ? current->ki : boot_kernel_instance;
	if (WARN_ON_ONCE(!p->ki))
		p->ki = boot_kernel_instance;
#endif
	p->flags &= ~PF_KTHREAD;
	if (args->kthread)
		p->flags |= PF_KTHREAD;
...
#ifdef CONFIG_KERNEL_INSTANCE
			//Per-intance task list
			{
				// Get the target list head, default: global init_task list
				struct list_head *head = p->ki ? p->ki->tasks_head : NULL;
				if (WARN_ON_ONCE(!head))
					head = &init_task.tasks;

				// Add the task to the target list
				list_add_tail_rcu(&p->tasks, head);
			}
#else
