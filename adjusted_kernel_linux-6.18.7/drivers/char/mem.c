			...
			size_t count, loff_t *ppos)
{
	...
	while (count > 0) {
		...
		err = -EPERM;
#ifdef CONFIG_KERNEL_INSTANCE
	// Check if the physical address is allowed for the current kernel instance
		if (!ki_devmem_phys_range_allowed(p, sz))
			goto failed;
#endif
...
#ifdef CONFIG_KERNEL_INSTANCE
	// Check if the physical address is allowed for the current kernel instance
	if (!ki_devmem_phys_range_allowed(offset, size))
		return -EPERM;
#endif
