...
// Definitions of the kernel instances
struct kernel_instance boot_ki;									// Boot instance
struct kernel_instance *boot_kernel_instance = &boot_ki;		// Pointer to the boot instance

struct kernel_instance ghost_ki;								// Ghost instance
struct kernel_instance *ghost_kernel_instance = &ghost_ki;		// Pointer to the ghost instance
...
// definition ghost cage borders
phys_addr_t ghost_ki_mem_start;

phys_addr_t ghost_ki_mem_size = 3300ULL * 1024 * 1024; // Ghost cage default size: 3300 MiB

// Ensures that the ghost cage is only computed once
static bool ki_ghost_cage_memblock_done __initdata;

void __init ki_compute_ghost_cage(void)
{
	// Get the end of the RAM
	phys_addr_t ram_end = memblock_end_of_DRAM();
	// Get the size of the ghost cage
	phys_addr_t size = ghost_ki_mem_size;
	// Get the alignment of the ghost cage to adapt its size to the page size
	const phys_addr_t align = 1ULL << 21; // 2 MiB

	// Check if the ghost cage has already been computed
	if (ki_ghost_cage_memblock_done)
		return;
	ki_ghost_cage_memblock_done = true;

	// Safety Check
	if (!ram_end || !size) {
		ghost_ki_mem_start = 0;
		ghost_ki_mem_size = 0;
		pr_warn("[BA Adjustments] KI: ghost cage disabled (no RAM or zero size)\n");
		return;
	}
	...
	// Ensure that minimum 1GiB outside the cage is reserved for the boot instance
	{
		const phys_addr_t boot_keep = 1ULL << 30;

		if (ram_end > boot_keep && size + boot_keep > ram_end)
			size = ALIGN_DOWN(ram_end - boot_keep, PAGE_SIZE);
	}

	// Check if the size is valid
	if (size < (phys_addr_t)PAGE_SIZE) {
		ghost_ki_mem_start = 0;
		ghost_ki_mem_size = 0;
		pr_warn("[BA Adjustments] KI: ghost cage disabled (size cap)\n");
		return;
	}

	// Set start address and size with alignment
	ghost_ki_mem_start = ALIGN_DOWN(ram_end - size, align);
	ghost_ki_mem_size = min(size, ram_end - ghost_ki_mem_start);

	// Lock memory block for the ghost cage
	if (memblock_reserve(ghost_ki_mem_start, ghost_ki_mem_size)) {
		ghost_ki_mem_start = 0;
		ghost_ki_mem_size = 0;
		pr_warn("[BA Adjustments] KI: ghost cage disabled (memblock_reserve failed)\n");
		return;
	}

	{
		// Provide some debug information
		phys_addr_t ghost_end = ghost_ki_mem_start + ghost_ki_mem_size;

		pr_info("[BA Adjustments] KI: ghost physical cage [%pa .. %pa) (%llu MiB) reserved\n",
			&ghost_ki_mem_start, &ghost_end,
			(unsigned long long)ghost_ki_mem_size >> 20);
	}
}
...
#ifdef CONFIG_KERNEL_INSTANCE
// Per-CPU mapping for the kernel instances
DEFINE_PER_CPU(struct kernel_instance *, this_ki);

static void __init init_kernel_instance_percpu(void)
{
	int cpu;

	for_each_possible_cpu(cpu) // Initialize the per-CPU mapping for the kernel instances
		per_cpu(this_ki, cpu) = boot_kernel_instance; // Set the boot instance for each CPU
	pr_info("[BA Adjustments] KI: per-CPU mapping initialized\n");
}
#endif
/*
...
void start_kernel(void)
{
	...
	#ifdef CONFIG_KERNEL_INSTANCE
	// Initialize the boot instance
	boot_ki.id = 0;
    boot_ki.current_task = NULL;
    boot_ki.tasks_head = &init_task.tasks;

    pr_info("[BA Adjustments] KI: boot instance initialized\n");

	// Initialize the ghost instance
	ghost_ki.id = 1;
	ghost_ki.current_task = NULL;
	ghost_ki.tasks_head = &init_task.tasks;

	pr_info("[BA Adjustments] KI: ghost instance initialized\n");
	#endif
	...
	// Initialize the per-CPU mapping for the kernel instances
	init_kernel_instance_percpu();
...
static int __ref kernel_init(void *unused)
{
	...
	pr_info("[BA Adjustments] KI: init path: rdinit=%s init=%s\n",
		ramdisk_execute_command ? ramdisk_execute_command : "(none)",
		execute_command ? execute_command : "(default)");
