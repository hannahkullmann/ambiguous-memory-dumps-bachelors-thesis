...
#include <linux/sched.h>       /* Wichtig für current-Makro */
#include <linux/mm.h>          /* Wichtig für page_to_pfn */
#include <asm/page.h>          /* Wichtig für PAGE_SHIFT */
				...
				fpi_t fpi_flags)
{
	...
	// Ensure that the cage pages never re-enter the global buddy free lists
	if (ki_page_in_ghost_cage(page)) {
		ki_free_ghost_pages(page, order);
		return;
	}
#endif
...
#ifdef CONFIG_KERNEL_INSTANCE
	// Force ghost-instance bulk callers down the slow path
	if (likely(system_state >= SYSTEM_SCHEDULING) && !in_interrupt() &&
	    current && READ_ONCE(current->ki) &&
	    READ_ONCE(current->ki)->id == 1)
		goto failed;
#endif
		...
		int preferred_nid, nodemask_t *nodemask)
{
	...
	// Ensure that scheduling is enabled and process exist	
	// Ensure that interrupt context is not active
	// Validate that the current process is a valid ghost process
	if (likely(system_state >= SYSTEM_SCHEDULING) && !in_interrupt() &&
	    current && READ_ONCE(current->ki) &&
	    READ_ONCE(current->ki)->id == 1) {
		page = ki_alloc_ghost_pages(gfp, order); // Allocate pages via ki_alloc_ghost_pages()

		// If allocation succeeded: manually perform post processing steps
		if (page) {
			post_alloc_hook(page, order, gfp); // Initializes page tracking, prepare kernel infrastructure, ...
			// If the allocation is a compound page, prepare the compound page
			if (order && (gfp & __GFP_COMP))
				prep_compound_page(page, order);
			return page;
		}

		// If allocation failed: log error and return NULL
		pr_warn_ratelimited("[BA Adjustments] ghost: pool exhausted, denying buddy fallback (order=%u)\n",
				    order);
		return NULL;
	}
#endif
...
// Freeing for ghost pages
#ifdef CONFIG_KERNEL_INSTANCE
	// Check if the page is in the ghost cage, then free 
	if (ki_page_in_ghost_cage(page)) {
		// Free the pages via ki_free_ghost_pages()
		ki_free_ghost_pages(page, order);
		return;
	}
#endif
